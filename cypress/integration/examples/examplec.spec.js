...
describe("Segundo conjunto de casos de pruebas avanzadas", function () {
  before(function () {
    ...
    cy.fixture("examplec").then(function (datos) {
      this.datos = datos;
  ...
  it("Lleanmos nuestro primer formulario utilizando data", function () {
    ...
    cy.get(".react-datepicker__day--0" + this.datos.fechaDeNacimiento[2])
      .should("be.visible")
      .click();
    ...
    cy.get(
      "div[class='custom-control custom-checkbox custom-control-inline']:has(label:contains('" +
        this.datos.hobbies[0] +
        "')) input"
    )
      .check({ force: true })
      .should("be.checked");
    ...
    cy.get("td:contains(Date of Birth) +td").should(
      "have.text",
      this.datos.fechaDeNacimiento[2] +
        " " +
    ...
    cy.get("td:contains(Hobbies) +td").should(
      "have.text",
      this.datos.hobbies[0] + ", " + this.datos.hobbies[1]
    );

    cy.get("td:contains(Picture) +td").should("have.text", this.datos.imagen);

    cy.get("td:contains(Address) +td").should(
      "have.text",
    ...
    cy.get("td:contains(State and City) +td").should(
      "have.text",
      this.datos.estado + " " + this.datos.ciudad
    );
  });
});
